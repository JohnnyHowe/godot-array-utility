...
func test_bothArraysEmpty_resultsIn_noDifference():
	...
	var to_remove := []
	var expected := []
	return TestCaseResult.contains_same_items(expected, ArrayUtility.ArraySetOperations.difference(source, to_remove))
...
func test_threeCustomSourceAndRemoveItems_withSameValue_withCustomOverride_resultsIn_noDifference():
	...
	return TestCaseResult.contains_same_items(expected, ArrayUtility.ArraySetOperations.difference(source, to_remove, equals_override))
